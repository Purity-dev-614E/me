...
function getStreaks(dates) {
  ...
  uniqueDates.sort().forEach((date) => {
    ...
    if (length) {
      ...
      while (dateSet.has(toKey(new Date(next.getTime() + 86_400_000)))) {
        run += 1;
...
export default function Journal() {
  const [entries, setEntries] = useState(
    RightNowData.journalEntries.map((entry) => ({
      ...entry,
      category: "Building",
    })),
  );
  const [total, setTotal] = useState(
    isSupabaseConfigured ? 0 : RightNowData.journalEntries.length,
  );
  ...
  useEffect(() => {
    ...
    let query = supabase
      ...
      .select("id, entry_date, body, category", { count: "exact" })
      .eq("published", true);
  ...
  return (
    <View>
      <div className={styles.page}>
        <a className={styles.back} href="#rn">
          <ArrowLeft size={18} /> Back to right now
        </a>
        <div className={styles.intro}>
          <div>
            ...
            <p className={styles.summary}>
              Small decisions, experiments, and lessons gathered while turning
              ideas into working software.
            </p>
          </div>
          <p className={styles.count}>
            <strong>{total}</strong> published{" "}
        ...
        <div className={styles.dashboard}>
          <div className={styles.streak}>
            <span>🔥</span>
            <p>
              <strong>{streaks.current} day</strong> current streak
            </p>
          </div>
          <div className={styles.streak}>
            <span>✦</span>
            <p>
              <strong>{streaks.longest} days</strong> longest streak
            </p>
          </div>
          <div
            className={styles.filters}
            aria-label="Filter journal entries by topic"
          >
            {categories.map((item) => (
              <button
                key={item}
                className={item === category ? styles.activeFilter : ""}
                onClick={() => {
                  setCategory(item);
                  setPage(1);
                }}
